...
async function getChannel(
  ...
  return api.get(`/channels/${channel_id}`);
}
...
async function createChannel({ title, type, password }: Props) {
  return api.post("/channels", { title, type, password });
}
...
async function updateChannel(
  ...
  if (type === "protected" && password !== "")
    return api.put(`/channels/${channel_id}`, { title, type, password });
  return api.put(`/channels/${channel_id}`, { title, type });
}

async function joinChannel(channel_id: number, password: string) {
  if (password != "")
    return api.post(`/channels/${channel_id}`, {
...
async function leaveChannel(channel_id: number) {
  return api.delete(`/channels/${channel_id}`);
}
